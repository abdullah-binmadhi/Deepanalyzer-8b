...
* Choose execution mode:
  * `[1] Single Script (.py)`: Paste the entire script generated by the AI.
...
* Prompts for output file name (e.g. `Clean_file.xlsx`). Saves clean, relational data to disk.
* Automatically writes the Power Query companion files (`powerquery_script.m` and `powerquery_guide.md`).
...
Every time a dataset is processed, DeepAnalyze generates:
1. `powerquery_script.m`: Ready-to-paste Power Query M-code.
2. `powerquery_guide.md`: A complete, click-by-click UI walkthrough with exact Excel formulas.
...
Every release is validated against 23 rigorous security and performance tests:
```bash
pytest
```
