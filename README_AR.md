...
### الخطوة 1: قراءة الملف والحفاظ التام على أعمدة إكسل المتعددة
...
* **الحفاظ على الأعمدة الـ 16+:** يستخدم محرك قراءة خاص لا يقتطع الأعمدة (`header=None`) لتفادي فقدان الأعمدة بسبب صفوف البيانات الوصفية في جداول تقارير الـ ERP.
...
### الخطوة 4: معمارية وهندسة ملف البيانات (السؤال 3)
* يسأل عن طبيعة التقرير:
  * `[1] جدول علائقي قياسي ونظيف (أعمدة واضحة)`
  * `[2] تقرير ERP هرمي غير مسطح (فواتير، دفاتر أستاذ، ترويسات متعددة)`
  * `[3] سجلات صحية EHR وملاحظات طبية سريرية`
  * `[4] لست متأكداً (كشف تلقائي)`
* **الفحص الذكي عند اختيار "لست متأكداً":** يحلل تكرار علامات النقطتين (`:`)، وعدد الأعمدة غير المسماة، والمسافات البادئة لتحديد ما إذا كان الجدول تقرير ERP متشعباً.

### الخطوة 5: الفحص العميق لكامل الملف وتصنيف الأنماط
* يفحص **كل صف وكل خلية** عبر الجدول بأكمله.
...
### الخطوة 6: استعراض عينات التشفير وفحص k-Anonymity
* يعرض جدولاً تفاعلياً يوضح الأنماط المكتشفة، وقيمها الأصلية، والرموز البديلة، والأعمدة المرتبطة بها للتأكد البصري.
* **تدقيق مخاطر كشف الهوية (k-Anonymity & l-Diversity):** يقيم دمج المؤشرات الشبه-معرّفة (العمر، الجنس، التواريخ، الرموز البريدية) ويحسب حجم أصغر فئة متكافئة ($k_{\min}$). ويطلق تحذيراً صريحاً إذا وجدت سجلات نادرة ($k < 5$) معرضة لهجمات الربط بقواعد بيانات أخرى.

### الخطوة 7: تدريب الأنماط التفاعلي وتخصيص التشفير
* يسأل المعالج: *"هل توجد أعمدة أو عناصر إضافية ترغب في تشفيرها؟ [y/N]"*
* عند الإجابة بـ **نعم**: أدخل اسم العمود ومثالاً لقيمته (مثل: العمود `Seq` بقيمة `10000`، أو `GL Code` بقيمة `500-000`).
* يستنتج ديب أنالايز التعبير النمطي (Regex) فورياً ويعيد تشفير كافة القيم المتطابقة عبر آلاف الصفوف خلال 34 مللي ثانية.

### الخطوة 8: تصدير الملف المشفر مقابل حمولة الحافظة بالخصوصية التفاضلية
* يسأل المعالج: *"هل ترغب في تنزيل ملف مطابق ومشفر بالكامل إلى القرص؟ [y/N]"*
  * **الخيار أ (الملف المشفر):** يصدّر `[اسم_الملف]_anonymized.xlsx` في نفس المجلد. يحتفظ بكامل هندسة وترتيب الصفوف والأعمدة الـ 16+، مع استبدال كافة الأسماء والأرقام ببدائل آمنة. **آمن تماماً للرفع المباشر إلى ChatGPT أو Claude أو Cursor.**
  * **الخيار ب (حمولة الحافظة):** ينسخ نموذجاً اصطناعياً من 5 صفوف مع إرشادات التوجيه إلى الحافظة. المتغيرات الرقمية تُحقن بـ **ضجيج خصوصية تفاضلية (Laplace noise)** بمعيار $\epsilon=1.0$ لجعل إعادة تركيب القيم الأصلية مستحيلة رياضياً.
...
* **الالتقاط التلقائي للبيانات:** يربط متغيرات الإدخال والإخراج (`df`، `data`، `INPUT_FILE`، `OUTPUT_FILE`)، وينفذ كتل `__name__ == '__main__'`، ويستخرج الجداول المحولة من الدوال أو المتغيرات داخل الذاكرة تلقائياً.
...
## 10. هيكلية الحزم البرمجية ومجموعة اختبارات التحقق الـ 37
...
### حزمة الاختبارات القبلية المؤتمتة (37 اختباراً)
يتم التحقق من كل إصدار من خلال 37 اختباراً دقيقاً للأمان والأداء:
```bash
pytest
```
