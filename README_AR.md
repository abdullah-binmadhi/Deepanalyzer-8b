...
2. [القدرات الجوهرية وهندسة النظام](#2-القدرات-الجوهرية-وهندسة-النظام)
   ...
   * [2.2 التفكيك الديناميكي الشامل لتقارير ERP وآلة الحالة لصفر فقدان بيانات](#22-التفكيك-الديناميكي-الشامل-لتقارير-erp-وآلة-الحالة-لصفر-فقدان-بيانات)
   * [2.3 طوبولوجيا مصنفات العمل ومحرك المخططات العلائقية ER متعدد الأوراق](#23-طوبولوجيا-مصنفات-العمل-ومحرك-المخططات-العلائقية-er-متعدد-الأوراق)
   * [2.4 شبكة العقول الـ 18 الإدراكية الشاملة](#24-شبكة-العقول-الـ-18-الإدراكية-الشاملة)
...
flowchart TD
    subgraph INGESTION["1. طبقة استقبال البيانات والاستكشاف الديناميكي"]
        RAW["ملف جدول أصلي / تصدير ERP خام<br/>(CSV, XLSX, TSV, Parquet)"]
        SNIFF["المستكشف الديناميكي الشامل لتخطيط الجداول<br/>(sniff_erp_layout: النمط، الترويسات، التسلسلات، السطور الملتفة)"]
        TOPOL["محرك طوبولوجيا أوراق العمل العلائقي<br/>(المفاتيح الأساسية، المرشحون للربط، مخطط ER متعدد الأوراق)"]
        RAW --> SNIFF
        SNIFF --> TOPOL
    end
    ...
    subgraph PAYLOADS["3. مسارات المخرجات والنسخ الآمن"]
        ...
        PQ_M["رفيق مايكروسوفت إكسل باور كويري الديناميكي<br/>(إزاحات محسوبة + كود M ديناميكي + دليل واجهة إكسل)"]
        GUIDE["دليل التوجيه التفاعلي خطوة بخطوة<br/>([dataset]_cleaning_prompt.md)"]
    ...
    subgraph AIRLOCK["5. غرفة العزل وجدار الحماية البرمجي AST"]
        ...
        STATE_FLAT["آلة الحالة لتسطيح الجداول بلا فقدان بيانات<br/>(erp_cleaner: ملء الترويسات للأسفل، دمج النصوص الملتفة)"]
        REPAIR["المعالجة الذاتية التفاعلية للأخطاء<br/>(التقاط الاستثناءات وإعادة التشغيل)"]
        
        CLOUD --> FIREWALL
        LOCAL --> FIREWALL
        FIREWALL -->|كود بايثون معتمد| SCOPE
        FIREWALL -->|المحرك الحتمي| STATE_FLAT
...
### 2.2 التفكيك الديناميكي الشامل لتقارير ERP وآلة الحالة لصفر فقدان بيانات
...
flowchart TD
    subgraph INPUT["ملف ERP خام وغير منظم"]
        M_TOP["صفوف البيانات الوصفية العلوية<br/>(عنوان التقرير، تواريخ الفلاتر، فراغات الإزاحة)"]
        M_DOC["ترويسات المستندات المتداخلة<br/>(رقم المستند، التاريخ، رمز العميل، الاسم، الإجمالي)"]
        M_LINE["بنود المعاملات التفصيلية<br/>(التسلسل، رمز الحساب، الوصف، الكمية، الوحدة، المبلغ)"]
        M_WRAP["صفوف النصوص الملتفة والمتعددة السطور<br/>(ملاحظات وتفاصيل ممتدة على سطور إضافية)"]
        M_SUB["السطور الفاصلة والإجماليات الفرعية<br/>(خطوط الفواصل، إجماليات الصفحات، الإجمالي العام)"]
        M_TOP --> M_DOC --> M_LINE --> M_WRAP --> M_SUB
    end

    subgraph SNIFFER["المرحلة 1: المستكشف الديناميكي الشامل (sniff_erp_layout)"]
        DET_ARCH["تصنيف النمط الهيكلي<br/>(SPARSE_DOCUMENT_HEADER مقابل REPEATING_BLOCK مقابل RAGGED_LEDGER)"]
        DET_KEYS["استكشاف أدوار الأعمدة عبر الإنتروبيا والأنماط<br/>* نمط رقم المستند التلقائي (اكتشاف البادئة دون تثبيت)<br/>* عمود التسلسل الرقمي<br/>* أعمدة التاريخ والعميل<br/>* عمود إجمالي الفاتورة"]
        DET_OFF["حساب إزاحة الترويسة ديناميكياً<br/>(تحديد بداية جدول المعاملات الفعلي بدون تخمين رقم ثابت)"]
    ...
    subgraph FLATTENER["المرحلة 2: آلة الحالة لصفر فقدان بيانات (clean_erp_report)"]
        ST_STATE["تتبع حالة المستند الحالية<br/>curr_master: {doc_no, doc_date, cust_code, cust_name, invoice_total}"]
        ST_FILL["ملء السياق للأسفل (Forward-Fill)<br/>ربط بيانات المستند الرئيسي بالبنود التفصيلية التابعة"]
        ST_WRAP["دمج وتوحيد السطور الملتفة (Wrap Stitcher)<br/>ضم الملاحظات الملتفة إلى بند المعاملة الأصلي"]
        ST_EVICT["استبعاد الفواصل والإجماليات ديناميكياً<br/>تنظيف فواصل الصفحات والملخصات دون حذف أي معاملة حقيقية"]
    ...
    subgraph DUAL_TRACK["المرحلة 3: التسليم المزدوج المؤسسي"]
        OUT_PY["المسار الأول: تنفيذ بايثون / بولارز في الذاكرة<br/>* جدول بيانات نظيف تماماً في RAM<br/>* الحفاظ على 100% من بنود المعاملات<br/>* صفر فقدان للبيانات من الصف الأول إلى الأخير"]
        OUT_PQ["المسار الثاني: كود باور كويري M الديناميكي<br/>* إزاحة محسوبة تلقائياً Table.Skip<br/>* تحديد ديناميكي دقيق لأنواع الأعمدة<br/>* تحديث بنقرة زر في إكسل"]
...
### 2.3 طوبولوجيا مصنفات العمل ومحرك المخططات العلائقية ER متعدد الأوراق
...
erDiagram
    WORKBOOK ||--|{ SHEET_A : يحتوي
    WORKBOOK ||--|{ SHEET_B : يحتوي
    WORKBOOK ||--|{ SHEET_C : يحتوي

    SHEET_A {
        string Primary_Key PK "فريد بنسبة 100% / بدون فراغات"
        date Transaction_Date "تاريخ بصيغة قياسية ISO"
        string Foreign_Key_B FK "مرجع للورقة SHEET_B"
        float Total_Amount "متحقق منه جبرياً"
    }

    SHEET_B {
        string Entity_ID PK "جدول البعد الرئيسي (Dimension)"
        string Entity_Name "مشفر / مستعاد الهوية"
        string Entity_Category "تصنيف منخفض الإنتروبيا"
    }

    SHEET_C {
        string Child_ID PK "جدول البنود التفصيلية (Ledger)"
        string Foreign_Key_A FK "مرجع للورقة SHEET_A بنسبة 1:N"
        float Line_Item_Amount "مطابق لإجمالي الفاتورة"
    }

    SHEET_A ||--o{ SHEET_C : "علاقة أب-ابن 1 إلى متعدد"
    SHEET_B ||--o{ SHEET_A : "علاقة بعد وبحث 1 إلى متعدد"
```

### 2.4 شبكة العقول الـ 18 الإدراكية الشاملة
...
# الترميم الذاتي المغلق لأوربوروس: تشخيص العطل وإصلاحه ذاتياً عبر نموذج 8B المحلي
%deepanalyze --fix "معالجة القيم الشاذة في total_amount"
...
### الخطوة 4: معمارية البيانات واكتشاف ترابط الصفحات ومخطط ER التفاعلي (السؤال 3)

* **نص التوجيه (Prompt)**: عرض **بطاقة معمارية المصنف (Workbook Topology Card)** و **مخطط ER العلائقي التفاعلي**؛ والسؤال عما إذا كان يرغب في معالجة كافة الصفحات معاً.
* **إجراء المحرك (Engine Action)**: فحص كافة أوراق العمل، وتحديد أدوار الصفحات (`TRANSACTION_LEDGER`، `LOOKUP_DIMENSION`)، واكتشاف المفاتيح الأساسية ومفاتيح الربط العلائقي التلقائي، وصفوف المجاميع الفرعية، وتوليد مخطط علاقات تفاعلي ديناميكي بصيغتي ASCII وMermaid لأي مصنف عمل دون تثبيت مسبق.
* **النتيجة المُسلّمة (Output)**: نطاق تنفيذي علائقي موحد ومتزامن لمصنف إكسل بالكامل.
...
### الخطوة 9: مسار التنظيف التوجيهي وغرفة العزل البرمجية

* **نص التوجيه (Prompt)**: تحديد مسار المعالجة والتنظيف:
  * `[1] التنظيف الحتمي في الذاكرة (آلة الحالة لصفر فقدان بيانات)`: تشغيل المنظف الحتمي الفوري (`erp_cleaner.py`) لملء الترويسات للأسفل ودمج السطور الملتفة وتصفية الفواصل دون أي حذف خاطئ للبنود.
  * `[2] دليل توجيهي للتنظيف خطوة بخطوة`: عرض وتصدير تعليمات دقيقة للنماذج السحابية (`[dataset]_cleaning_prompt.md`) تشرح خوارزمية المعالجة الدقيقة لبايثون أو باور كويري M.
  * `[3] كود باور كويري M ديناميكي`: توليد كود لغة M مع إزاحة محسوبة تلقائياً Table.Skip وأنواع بيانات مدققة لمصنف إكسل.
* **بوابة الربط السحابي المباشر (Frontier API Gateway - BYOK)**: في حال اكتشاف مفاتيح واجهة برمجة التطبيقات (OpenAI, Anthropic, OpenRouter, وغيرها)، يتيح ديب أنالايز **التنفيذ السحابي المباشر بضغطة زر واحدة** بعد فحص أمني حتمي خالي من PII بنسبة 100%.
...
يعتمد محرك باور كويري الديناميكي (`powerquery.py`) على المستكشف الشامل لتخطيط الجداول (`sniff_erp_layout`) لاحتساب إزاحة صفوف الترويسات الحقيقية تلقائياً (`header_row_idx`)، واكتشاف بادئات الأنماط ديناميكياً، وتوليد كود لغة M مرن يتكيف مع بنية أي جدول دون تثبيت مسبق للإزاحات أو أسماء الأعمدة.

عند اختيار نمط التسليم `[3] Power Query (M-Code)` في الخطوة 9، يولد النظام تلقائياً:

1. `powerquery_script.m`: كود لغة M ديناميكي جاهز للصق في إكسل مباشرة.
...
let
    ...
    // 5. استخراج ترويسات المستندات باكتشاف البادئة الديناميكية
    ...
    // 8. اختيار وإعادة تسمية الأعمدة النهائية وفرض الأنواع ديناميكياً
...
| `--fix` | بيئة جوبيتر / آي بايثون | تشخيص العطل جراحياً وترميمه آلياً بنموذج 8B المحلي أو بتوجيه مخصص | `%deepanalyze --fix "معالجة القيم الشاذة"` |
...
├── erp_cleaner.py   # المستكشف الديناميكي الشامل لتقارير ERP وآلة الحالة لصفر فقدان بيانات
├── cockpit_tui.py   # قمرة القيادة التفاعلية النصية بالكامل (Textual TUI & TCSS)
├── cockpit.py       # لوحة معلومات ANSI التفاعلية ومفتش تحول الأعمدة
├── frontier.py      # بوابة الربط السحابي المباشر (BYOK) مع ضمان 0% PII
├── data_engineering.py # محرك استكشاف الميزات والتعزيز التنبؤي في Polars
├── transformer.py   # محركات التسطيح الحتمية لتقارير الـ ERP المعقدة
├── magics.py        # توجيهات آي بايثون السريعة (%deepanalyze, --airgap, --run, --fix, --undo, --audit)
├── client.py        # عميل الاستدلال المحلي، وفاحص الحالة، ومولد الترميم الذاتي
└── server.py        # خادم الاستدلال المحلي وإدارة نماذج GGUF (Metal/CUDA/Socket)
```

### حزمة الاختبارات القبلية المؤتمتة (167 اختباراً)
يتم التحقق من كل إصدار من خلال 167 اختباراً دقيقاً للأمان والأداء ومجلس العقول الـ 18 ثنائية اللغة:
```bash
pytest
```

* `tests/test_benchmarks.py`: التحقق من كافة معايير الأمان والخصوصية الـ 11 عبر المستويين: المستوى الأول Tier 1 والمستوى الثاني Tier 2، مع التحقق من تضمينها التلقائي في وثيقة `compliance_audit.md`.
* `tests/test_erp_cleaner.py`: التحقق من المستكشف الديناميكي الشامل لتخطيط الجداول (`sniff_erp_layout`)، واكتشاف البادئات التلقائي، وآلة الحالة لصفر فقدان بيانات، ودمج السطور الملتفة عبر مختلف هياكل تقارير ERP.
