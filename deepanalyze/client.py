...
def check_model_health(server_url: Optional[str] = None, timeout: float = 0.5) -> bool:
    ...
    base_url = server_url.rstrip("/") if server_url else get_default_server_url()
    test_url = f"{base_url}/health"
    try:
        req = urllib.request.Request(
            test_url,
            headers={"User-Agent": "DeepAnalyze-Client/4.0"}
        )
...
def request_model_fix(
    ...
    req = urllib.request.Request(
        completion_url,
        data=req_data,
        headers={"Content-Type": "application/json", "User-Agent": "DeepAnalyze-Client/4.0"}
    )
