...
def build_master_prompt(
    ...
    if arch_key == "ERP_RAGGED":
        ...
        sections.append(
            ...
            "  • **CRITICAL INSTRUCTION:** Do NOT call `df.iloc[18:]` or `Table.Skip(18)`! Hardcoded row skips discard valid early invoices (e.g. `IV-11319`). Use state-machine row-by-row parsing."
        )
    ...
    if arch_key == "ERP_RAGGED":
        erp_blueprint = (
            ...
            "```python\n"
            "# Production-grade state machine for unflattened master-detail ERP exports:\n"
            "import re\n"
            "import pandas as pd\n"
            "import numpy as np\n"
            "\n"
            "def normalize_erp_report(raw_df: pd.DataFrame) -> pd.DataFrame:\n"
            "    records = []\n"
            "    curr_master = None\n"
            "    curr_line = None\n"
            "    doc_regex = re.compile(r'^(IV|INV|CN|DN)-\\d+', re.IGNORECASE)\n"
            "    \n"
            ...
            "        \n"
            "        # 1. Detect Document Header (Archetype A)\n"
            ...
            "            curr_master = {\n"
            "                'doc_no': val0,\n"
            "                'doc_date': str(row.iloc[2]).split()[0] if pd.notna(row.iloc[2]) else None,\n"
            "                'customer_code': str(row.iloc[4]).strip() if pd.notna(row.iloc[4]) else None,\n"
            "                'customer_name': str(row.iloc[6] if pd.notna(row.iloc[6]) else row.iloc[7]).strip(),\n"
            "                'invoice_total': float(str(row.iloc[15]).replace(',', '')) if pd.notna(row.iloc[15]) else None\n"
            "            }\n"
            "            continue\n"
            "            \n"
            "        # 2. Skip Noise & Subtotals (Archetype D)\n"
            "        if any(k in val0 for k in ['Doc. No', 'Seq', 'Account Summary', 'WEST MALAYAN', 'Grand Total']) or \\\n"
            ...
            "        try:\n"
            "            s_num = float(val0)\n"
            "            if s_num >= 1000 and (val1 == '500-000' or len(val1) > 0):\n"
            "                is_seq = True\n"
            "        except ValueError:\n"
            "            is_seq = False\n"
            "            \n"
            ...
            "                'Quantity': float(str(row.iloc[10]).replace(',', '')) if pd.notna(row.iloc[10]) else 1.0,\n"
            "                'UOM': str(row.iloc[11]).strip() if pd.notna(row.iloc[11]) else 'CTN',\n"
            "                'Unit Price': float(str(row.iloc[12]).replace(',', '')) if pd.notna(row.iloc[12]) else 0.0,\n"
            "                'Item Amount': float(str(row.iloc[13]).replace(',', '')) if pd.notna(row.iloc[13]) else 0.0\n"
            "            }\n"
            "            continue\n"
            "            \n"
