...
def build_master_prompt(
    ...
    if arch_key == "ERP_RAGGED":
        ...
        sections.append(
            "- **Structural Archetype Classification (ERP Hierarchical Matrix):**\n"
            "  • **Archetype A (Nested Block Header):** Header rows (Doc. No, Date, Customer Code/Name, Total) occupy sparse rows where rightward numeric cells are blank. Must be captured into state and forward-filled down across child line items.\n"
            "  • **Archetype B (Multi-Line Item Wrap):** Line item descriptions wrap onto 2-3 subsequent physical rows where sequence/numeric keys are null. Must be concatenated to the active line item's `Full_Description`.\n"
            "  • **Archetype C (Sparse Hierarchical Index):** Intermittent document groupings requiring stateful forward-fill (`ffill()`).\n"
            "  • **Archetype D (Interleaved Subtotals & Noise):** Repeated page headers (`Page 1 of 69`), dashed separators, and summary rows (`Grand Total`, `Account Summary`). Must be filtered dynamically without hardcoded index row slicing.\n"
            "  • **CRITICAL INSTRUCTION:** Do NOT use hardcoded row index skips (e.g. `df.iloc[N:]` or `Table.Skip(N)`). Arbitrary row slicing discards valid early records and transactions. Use state-machine row-by-row parsing or condition-based filtering."
        )
    ...
    if arch_key == "ERP_RAGGED":
        sections.append(
            ...
            "   - Parse the raw table maintaining state: active document identifier, document date, entity/customer attributes, and transaction totals.\n"
            "   - When a row contains a document header (matching document identifier pattern or keywords), update active master header fields.\n"
            "   - When a row contains a detail line item (valid sequence number or non-null item metrics/quantities), create a line item record merged with the current active master header fields (forward-filling master attributes).\n"
            "   - When a continuation row appears (description text with empty sequence/price), concatenate its text into the preceding line item's description.\n"
            "   - Filter out duplicate report titles, column header repeats, and summary/subtotal rows.\n"
            "   - Standardize flattened dataset by combining discovered master attributes with child line attributes."
        )
        step_num += 1
    ...
    if arch_key == "ERP_RAGGED":
        ...
        col_doc = f"row.iloc[{schema.doc_col}]"
        col_date = f"row.iloc[{schema.date_col}]" if schema.date_col is not None else "None"
        col_code = f"row.iloc[{schema.entity_code_col}]" if schema.entity_code_col is not None else "None"
        col_name = f"row.iloc[{schema.entity_name_col}]" if schema.entity_name_col is not None else "None"
        col_total = f"row.iloc[{schema.total_col}]" if schema.total_col is not None else "None"
        col_desc = f"row.iloc[{schema.desc_col}]"
        col_qty = f"row.iloc[{schema.qty_col}]" if schema.qty_col is not None else "None"
        col_uom = f"row.iloc[{schema.uom_col}]" if schema.uom_col is not None else "None"
        col_price = f"row.iloc[{schema.price_col}]" if schema.price_col is not None else "None"
        col_amt = f"row.iloc[{schema.amount_col}]" if schema.amount_col is not None else "None"
        col_item_code = f"row.iloc[{schema.item_code_col}]" if schema.item_code_col is not None else "None"

        erp_blueprint = (
            f"\n\n**STATE-MACHINE PATTERN FOR HIERARCHICAL ERP REPORT DECONSTRUCTION (PANDAS):**\n"
            f"```python\n"
            f"# Production-grade state machine for unflattened master-detail ERP exports (Dynamically Sniffed):\n"
            f"import re\n"
            f"import pandas as pd\n"
            f"import numpy as np\n"
            f"\n"
            f"def normalize_erp_report(raw_df: pd.DataFrame) -> pd.DataFrame:\n"
            f"    records = []\n"
            f"    curr_master = None\n"
            f"    curr_line = None\n"
            f"    doc_regex = re.compile(r'{schema.doc_regex_pattern}', re.IGNORECASE)\n"
            f"    \n"
            f"    for idx, row in raw_df.iterrows():\n"
            f"        val_doc = str({col_doc}).strip() if pd.notna({col_doc}) else ''\n"
            f"        val_desc = str({col_desc}).strip() if pd.notna({col_desc}) else ''\n"
            f"        row_str = ' '.join([str(v) for v in row if pd.notna(v)])\n"
            f"        \n"
            f"        # 1. Detect Document Header (Archetype A)\n"
            f"        if doc_regex.match(val_doc):\n"
            f"            if curr_line is not None:\n"
            f"                records.append(curr_line)\n"
            f"                curr_line = None\n"
            f"            curr_master = {{\n"
            f"                '{schema.doc_name}': val_doc,\n"
            f"                '{schema.date_name}': str({col_date}).split()[0] if {col_date} is not None and pd.notna({col_date}) else None,\n"
            f"                '{schema.entity_code_name}': str({col_code}).strip() if {col_code} is not None and pd.notna({col_code}) else None,\n"
            f"                '{schema.entity_name_name}': str({col_name}).strip() if {col_name} is not None and pd.notna({col_name}) else None,\n"
            f"                '{schema.total_name}': float(str({col_total}).replace(',', '').replace('$', '')) if {col_total} is not None and pd.notna({col_total}) else None\n"
            f"            }}\n"
            f"            continue\n"
            f"            \n"
            f"        # 2. Skip Noise & Subtotals (Archetype D)\n"
            f"        if any(k.lower() in row_str.lower() for k in {schema.noise_keywords}) or 'page ' in row_str.lower():\n"
            f"            if curr_line is not None:\n"
            f"                records.append(curr_line)\n"
            f"                curr_line = None\n"
            f"            continue\n"
            f"            \n"
            f"        # 3. Detect Line Item (Level 2 Child)\n"
            f"        is_seq = False\n"
            f"        seq_num = 1000\n"
            f"        try:\n"
            f"            s_num = float(val_doc)\n"
            f"            if s_num >= 1000 or (s_num.is_integer() and 1 <= s_num <= 5000):\n"
            f"                is_seq = True\n"
            f"                seq_num = int(s_num)\n"
            f"        except ValueError:\n"
            f"            is_seq = False\n"
            f"            \n"
            f"        has_qty = {col_qty} is not None and pd.notna({col_qty})\n"
            f"        if (is_seq or has_qty) and curr_master is not None and val_desc:\n"
            f"            if curr_line is not None:\n"
            f"                records.append(curr_line)\n"
            f"            curr_line = {{\n"
            f"                **curr_master,\n"
            f"                '{schema.seq_name}': seq_num,\n"
            f"                '{schema.item_code_name}': str({col_item_code}).strip() if {col_item_code} is not None and pd.notna({col_item_code}) else None,\n"
            f"                '{schema.desc_name}': val_desc,\n"
            f"                '{schema.qty_name}': float(str({col_qty}).replace(',', '')) if {col_qty} is not None and pd.notna({col_qty}) else 1.0,\n"
            f"                '{schema.uom_name}': str({col_uom}).strip() if {col_uom} is not None and pd.notna({col_uom}) else None,\n"
            f"                '{schema.price_name}': float(str({col_price}).replace(',', '')) if {col_price} is not None and pd.notna({col_price}) else 0.0,\n"
            f"                '{schema.amount_name}': float(str({col_amt}).replace(',', '')) if {col_amt} is not None and pd.notna({col_amt}) else 0.0\n"
            f"            }}\n"
            f"            continue\n"
            f"            \n"
            f"        # 4. Detect Multi-Line Description Wrap (Archetype B)\n"
            f"        if curr_line is not None and val_desc and not is_seq and not has_qty:\n"
            f"            curr_line['{schema.desc_name}'] += f' {{val_desc}}'\n"
            f"                \n"
            f"    if curr_line is not None:\n"
            f"        records.append(curr_line)\n"
            f"        \n"
            f"    clean_df = pd.DataFrame(records)\n"
            f"    return clean_df\n"
            f"\n"
            f"df = normalize_erp_report(df)\n"
            f"```"
        )
